...
class MingleComposer(QDialog):
    def __init__(self, mail_content="", *args, **kwargs):
        ...
        # Add bullet points actions to the toolbar
        bullet_points_icon = QIcon("UI-IMAGES/bullet-points.png")
        self.bullet_points_action = QAction(bullet_points_icon, "Bullet Points", self)
        self.bullet_points_action.triggered.connect(self.insert_bullet_points)
        format_toolbar.addAction(self.bullet_points_action)

        ordered_list_icon = QIcon("UI-IMAGES/ordered-list.png")
        self.ordered_list_action = QAction(ordered_list_icon, "Ordered List", self)
        self.ordered_list_action.triggered.connect(self.insert_ordered_list)
        format_toolbar.addAction(self.ordered_list_action)
    ...
    def toggle_bullet_points(self, style):
        cursor = self.mail_composer.textCursor()
        current_list = cursor.currentList()
        if current_list:
            cursor.createList(QTextListFormat.Style.ListDisc if current_list.format().style() != style else QTextListFormat.Style.ListDecimal)
        else:
            cursor.insertList(style)


    def insert_bullet_points(self):
        cursor = self.mail_composer.textCursor()
        if cursor.hasSelection():
            ...
            self.toggle_bullet_points(QTextListFormat.Style.ListDisc)
    ...
    def insert_ordered_list(self):
        ...
        if cursor.hasSelection():
            ...
            self.toggle_bullet_points(QTextListFormat.Style.ListDecimal)
